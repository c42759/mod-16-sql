...
function dump() {
	// SOURCE :: http://www.plux.co.uk/backing-up-a-mysql-database-with-php-using-mysqli/
	try {
		...
		print("-- BO".$cfg->system->version."-".$cfg->system->sub_version." Dump\n");
		print("-- Server version:".$db->server_info."\n");
		print("-- Generated: ".date('Y-m-d h:i:s')."\n");
		print('-- Current PHP version: '.phpversion()."\n");
		print('-- Host: '.$cfg->db->host."\n");
		print('-- Database:'.$cfg->db->database."\n");
		...
		if (!$res_tables = $db->query($strSQL))
			throw new Exception("MySQL Error: " . $db->error . 'SQL: '.$strSQL);
